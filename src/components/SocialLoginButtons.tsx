...
export function SocialLoginButtons({ action = '로그인', className = '' }: SocialLoginButtonsProps) {
  ...
  // 둘 다 미설정이면 렌더링 안 함
  if (!kakaoClientId && !googleClientId) return null;
  ...
  const handleKakaoLogin = () => {
    ...
    window.location.href = `https://kauth.kakao.com/oauth/authorize?client_id=${kakaoClientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&state=${state}&scope=profile_nickname,account_email`;
  };
  ...
  const handleGoogleLogin = () => {
    ...
    window.location.href = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${googleClientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=openid+email+profile&state=${state}&access_type=offline&prompt=consent`;
  };
      ...
      <div className="flex flex-col gap-2">
        ...
        {googleClientId && (
          <button
            ...
            disabled={isLoading !== null}
            className="w-full flex items-center justify-center gap-2 h-[48px] rounded-[0.75rem] font-medium text-sm border border-gray-200 bg-white text-gray-700 transition-all hover:bg-gray-50 disabled:opacity-50"
          >
            {isLoading === 'google' ? (
